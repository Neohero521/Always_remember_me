...
// ============================================================
// 酒馆助手（Tavern Helper）脚本兼容层
// 本文件由原 SillyTavern 扩展（manifest.json + index.js）转换而来。
// 原扩展通过 extension_settings / eventSource / getContext 等 ST 内部 API
// 与酒馆交互；此处用酒馆助手提供的接口（脚本变量、tavern_events、
// SillyTavern 全局对象）构建同名 shim，使下方业务逻辑无需改动。
// ============================================================

// 内嵌资源：HTML 模板与样式（webpack 以字符串形式导入）
import uiTemplate from './ui-template.html';
import './styles.css';
import * as PromptConstants from './prompt-constants';

// ---------- 事件系统：直接复用酒馆原生 eventSource / eventTypes ----------
// 酒馆助手脚本运行在无沙盒 iframe 中，window.$ === window.parent.$，
// 且 SillyTavern 全局对象可直接访问，故 eventSource 与 event_types 可直接别名。
...
// 原扩展把所有配置写入 extension_settings[extensionName] 并靠
// saveSettingsDebounced() 持久化；这里改为读写"脚本变量"，
// 对外仍暴露同名的 extension_settings / saveSettingsDebounced。
const extensionName = 'Always_remember_me';
const extensionFolderPath = ''; // 脚本形式不再需要文件夹路径，保留常量以免下游引用报错
...
// 防抖持久化，模仿原 saveSettingsDebounced 的行为
const _saveTimer = { id: null as ReturnType<typeof setTimeout> | null };
...
// 赋值 [extensionName] 时更新缓存并触发持久化。
// 业务代码以 extension_settings[extensionName].xxx = yyy 形式直接修改对象属性，
// 因 _settingsCache 是同一引用，修改会反映到缓存，再由 saveSettingsDebounced 写回。
const extension_settings: Record<string, any> = new Proxy({} as Record<string, any>, {
...
// ---------- DOM 上下文：把 document 指向父酒馆页面 ----------
// 脚本运行在无沙盒 iframe 中，但 UI（悬浮球/面板/阅读器）通过 $("body").append
// 注入到父酒馆页面。下方业务代码大量使用 document.getElementById / addEventListener
// 等原生 API，必须指向父页面文档才能找到这些元素。
// 先用 window.document（属性访问，不受下方局部变量影响）保存 iframe 文档引用，
// 供 teleportStyle 选取 iframe 内 <style> 使用。
const _iframeDocument = window.document;
// 用 const 在模块作用域覆盖全局 document，使下方所有 document.xxx 指向父页面
const document = window.parent.document as Document;

// ---------- 样式传送：把 iframe 内的 <style> 复制到酒馆父页面 <head> ----------
// 脚本中 import './styles.css' 只会在 iframe 内生效，而悬浮球/面板被
// appendTo('body')（即父页面 body）渲染，故需把样式传送到父页面。
let _styleTeleportDestroy: (() => void) | null = null;
function teleportStyle() {
    const $container = $('<div>')
        .attr('script_id', getScriptId())
        .append($('head > style', _iframeDocument).clone())
        .appendTo('head'); // 父页面 head
    _styleTeleportDestroy = () => $container.remove();
}
function deteleportStyle() {
    if (_styleTeleportDestroy) {
        _styleTeleportDestroy();
        _styleTeleportDestroy = null;
    }
}
...
// ==============================================主程序开始==============================================
// 注：extensionName / extensionFolderPath 已在文件顶部兼容层中定义
...
jQuery(async () => {
    // 预加载脚本变量到缓存，确保 extension_settings 立即可用
    _loadSettingsCache();

    try {
        // HTML 模板已通过 import 内嵌，无需再走 HTTP 拉取
    ...
    // 将 iframe 内的样式传送到酒馆父页面，使悬浮球/面板样式生效
    teleportStyle();
