 ...
 * 运行形式：单文件 JS，直接导入酒馆助手脚本库即可使用
 * 原始项目：SillyTavern 扩展 Always_remember_me v2.3.1 (by Neohero521)
 * 转换说明：将原扩展的 manifest.json + index.js + style.css + example.html
 *           + prompt-constants.js 合并为单文件，用酒馆助手 API 替代 ST 内部接口
 *
 * 文件分块索引：
...
/* ============================================================
 * ▌SECTION 0  脚本元信息（必须先声明，后续兜底函数会引用 SCRIPT_ID）
 * ============================================================ */
...
/* ============================================================
 * ▌SECTION 0.5  运行环境补全（惰性安全获取）
 * ------------------------------------------------------------
 * ⚠️ 致命警告：
 *   参考脚本（时之写卡器）在 IIFE 顶层从不访问 window.parent.xxx，
 *   只在每个函数内部用三元惰性判断。
 *   如果顶层直接访问 `window.parent.document` / `window.parent.$`，
 *   在严格模式下的某些 iframe 环境（跨域 / sandbox / 严格 CSP）
 *   会直接抛出 SecurityError / TypeError，整个 IIFE 立即崩溃，
 *   连上面的 console.log 都来不及执行。
 *
 * 因此本 SECTION 的所有"父页面全局"都改为：
 *   - 顶层用 `__safeGet(...)` 惰性取值（try/catch 兜底 + 空实现）
 *   - `document` / `$` 等改为 let，调用 `__bootInit()` 后再赋值
 *     （`__bootInit` 在 jQuery ready 时机执行，此时父页面一定就绪）
 * ============================================================ */

// 安全获取工具：返回 valueGetter() 的值，抛错则返回 fallback
...
// 顶层阶段：只使用 iframe 自身的全局（或空实现兜底），绝对不碰 window.parent
...
// document 默认 = 自身 iframe 的 document（空壳也行，UI 注入前一定会再重绑）
var _Doc  = __safeGet(function(){ return (typeof document !== 'undefined') ? document : (window && window.document) ? window.document : null; }, null);

// IIFE 作用域局部变量（遮蔽同名全局），先用 iframe 自身的值
var jQuery = _jQ;
var $      = _Dol;
var toastr = _Tst;
var document = _Doc;
...
function getVariables() {
    ...
    try {
        var ST = __safeGet(function(){ return (typeof SillyTavern !== 'undefined') ? SillyTavern : (window && window.SillyTavern); }, null);
        if (ST && typeof ST.getVariables === 'function') return ST.getVariables.apply(ST, arguments);
    } catch(_){}
    try {
        var TH = __safeGet(function(){ return (typeof TavernHelper !== 'undefined') ? TavernHelper : (window && window.TavernHelper); }, null);
        if (TH && typeof TH.getVariables === 'function') return TH.getVariables.apply(TH, arguments);
    } catch(_){}
    try { var s = window && window.localStorage ? window.localStorage.getItem(SCRIPT_ID + ':vars') : null; return s ? JSON.parse(s) : {}; } catch(_){ return {}; }
}
function replaceVariables(obj) {
    ...
    try {
        var ST = __safeGet(function(){ return (typeof SillyTavern !== 'undefined') ? SillyTavern : (window && window.SillyTavern); }, null);
        if (ST && typeof ST.replaceVariables === 'function') return ST.replaceVariables.apply(ST, arguments);
    } catch(_){}
    try {
        var TH = __safeGet(function(){ return (typeof TavernHelper !== 'undefined') ? TavernHelper : (window && window.TavernHelper); }, null);
        if (TH && typeof TH.replaceVariables === 'function') return TH.replaceVariables.apply(TH, arguments);
    } catch(_){}
    try { if (window && window.localStorage) window.localStorage.setItem(SCRIPT_ID + ':vars', JSON.stringify(obj || {})); } catch(_){}
}
...
// ---------- 引导初始化：延迟到 jQuery ready 时机才重绑 $/document 到父页面 ----------
// 参考时之写卡器：任何访问父页面属性的操作都放进函数里 + 三元判断
function __bootInit() {
    try { console.log('[小说续写插件] ⚙️ __bootInit 开始：尝试重绑 $/document 到父页面'); } catch(_){}
    // 父页面引用（全部惰性安全获取，永远不直接赋值 = window.parent.xxx）
    var pWin  = __safeGet(function(){ return (typeof window !== 'undefined' && window.parent) ? window.parent : null; }, null);
    var pDoc  = __safeGet(function(){ return (pWin && pWin.document) ? pWin.document : ((typeof window !== 'undefined' && window.document) ? window.document : document); }, document);
    ...
    // 最终兜底：无论如何执行入口
    try { novelWriterEntryPoint(); } catch (e) { try{console.error('[小说续写插件] __bootInit → novelWriterEntryPoint 失败:', e);}catch(_){} }
}
...
function saveSettingsDebounced() {
    if (_saveTimer.id !== null) clearTimeout(_saveTimer.id);
    _saveTimer.id = setTimeout(_persistSettings, 300);
}
 ...
 * ------------------------------------------------------------
 * 酒馆助手脚本规范：
 *   1) 脚本启动 → 注册脚本按钮（用户在脚本库点按钮才打开 UI）
 *   2) 同时兜底添加浮动按钮（若 eventOn/getButtonEvent 不可用）
 *   3) pagehide 时统一清理所有注入的 DOM
 * ============================================================ */
...
function cleanupNovelWriter() {
    try {
        // 销毁悬浮球（如已初始化）
        try {
            if (typeof FloatBall !== 'undefined' && FloatBall.destroy) {
                FloatBall.destroy();
            }
        } catch (_) {}
        // 移除注入的 UI 根容器与样式
...
/* ============================================================
 * ▌SECTION 4  入口 & 卸载清理（已完成）
 * ------------------------------------------------------------
 * 入口：novelWriterEntryPoint() → 注册脚本按钮 / 兜底浮动按钮
 *      用户点击按钮 → openNovelWriter() → 创建 iframe 并注入 UI
